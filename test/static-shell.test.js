...
test('includes a looping pixel heart animation demo', async () => {
  ...
  assert.doesNotMatch(demo, /const expansion =/);
  assert.match(demo, /draw\(0\);/);
  assert.match(demo, /const breathingSize = \.52 \+ \(Math\.sin\(seconds \* 1\.25 - Math\.PI \/ 2\) \+ 1\) \* \.24;/);
  assert.match(demo, /const pulseX = x \/ breathingSize;/);
  assert.match(demo, /const heartFill = Math\.max\(0, Math\.min\(1, \.5 - heartDistance\(pulseX, pulseY\) \* 8\)\);/);
